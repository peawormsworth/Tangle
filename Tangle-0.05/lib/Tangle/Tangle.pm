...
sub cnot {
   ...
   #my $q1 = HALF_ROOT * Tangle->new(1,1);
   #my $q1 = 1/sqrt(4) * Tangle->new(1,1,1,1);
   #my $gate = Tangle->new(0,0,1,-1);
   #my $gate = Tangle->new(0,0,1,-1);
   #my $cnot = 1 / $gate * $tensor->conjugate * $gate;
   #my $cnot = $gate * $tensor * 1/$gate;
   #
   #my $cnot = $tensor;
...
sub swap {
   ...
   #my $gate = Tangle->new(0,0,1,-1);
   my $gate = Tangle->new(0,1);
   my $swap =  $gate * $m->conjugate;
   $m->_gate($swap);
}
...
sub h     { my $m = shift; $m->_gate(HALF_ROOT * (ref $m)->new( 1,  1 ) / $m) }
...
# This would be called shift() if the word was not reserved by perl ...
sub sift { die 'incomplete function placeholder' }
...
#
# create a new object - primarily in the CayleyDickson parent package.
# expects 2 (or 2^n) parameters of numbers of objects
# and then creates a triple reference between a/b nodes
#

sub prepare { shift; map(\\\$_,@_)  }
